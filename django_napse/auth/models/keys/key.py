...
class NapseAPIKey(APIKey):
    ...
    def save(self, *args, **kwargs) -> None:
        ...
        if self.is_master_key and NapseAPIKey.objects.filter(is_master_key=True).count() > 0:
    ...
    def add_permission(self, space, permission):
        return KeyPermission.objects.create(key=self, space=space, permission_type=permission)
